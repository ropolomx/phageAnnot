...
for r in recs:
    for feature in r.features:
        if feature.type == "CDS":
            headers.append('_'.join([r.name, feature.qualifiers['locus_tag'][0], feature.qualifiers['product'][0]]))
            translations.append(feature.qualifiers['translation'][0])

# Zip headers and translated sequences into a zip object with tuples
# Returns an iterator of tuples

fastaTuple = zip(headers, translations) 
...
newRecs = [SeqRecord(Seq(f[1], IUPAC.protein),id=f[0], name=f[0]) for f in fastaTuple]
